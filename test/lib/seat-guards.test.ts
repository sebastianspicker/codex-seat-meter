...
describe("isCodexUsageApiResponse", () => {
  it("returns false for an empty object (rate_limit required)", () => {
  ...
  it("returns false when rate_limit is null", () => {
    expect(isCodexUsageApiResponse({ rate_limit: null })).toBe(false);
  });
});
