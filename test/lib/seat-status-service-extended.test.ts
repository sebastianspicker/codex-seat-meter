...
describe("fetchSeatStatus", () => {
  ...
  it("returns 400 when no access token found", async () => {
    ...
    expect(result.status).toBe(400);
    expect(result.body).toEqual({ ok: false, error: "No access token found in auth file. Ensure the file contains tokens.access_token or OPENAI_API_KEY." });
  });
  ...
  it("returns 200 with defaults for response without rate_limit", async () => {
    mockLoadSeatAuth.mockResolvedValue({ tokens: { access_token: "tok" } });
    mockFetchUsage.mockResolvedValue({
      ok: true,
